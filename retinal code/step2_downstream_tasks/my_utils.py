...
class MyDataset(data.Dataset):
    ...
    def __getitem__(self,idex):
        ...
        #read image
        image_instance=Image.open(os.path.join(self.data_path,data_instance[0]))
        label_instance=Image.open(os.path.join(self.data_path,data_instance[1]))
        #augmentation
        if self.if_augmentation:
            ...
            # rotation
            rotation_degree=random.uniform(-180,180)
        ...
        image_instance=np.transpose(image_instance,(2,0,1))
...
if __name__=='__main__':
    import cv2
    main_path='../../datasets/retinal_images'
    segmenting_object='Vessel'
    dataset_name='HRF'
    training_data_list,test_data_list=np.load(os.path.join(main_path,segmenting_object+'_'+dataset_name+'.npy'),allow_pickle=True)
    ...
    for index in range(len(batch_data)):
        ...
        data_instance=np.floor((data_instance*[0.229, 0.224, 0.225]+[0.485, 0.456, 0.406])*255)
